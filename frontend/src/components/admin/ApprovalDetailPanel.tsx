/**
 * Approval Detail Panel - Inline Detail View for Split Layout
 *
 * Shows all details from all 3 levels:
 ...
 *
 * Includes approve/reject/disapprove functionality at the bottom
 */
...
interface ApprovalDetailPanelProps {
  ...
  // Close callback for the panel
  onClose?: () => void;
  // Actions for pending scripts
  onApprove?: (data: ReviewAnalysisData) => void;
  onReject?: (data: ReviewAnalysisData) => void;
  // Action for approved scripts
  onDisapprove?: (reason: string) => void;
  // Actions for shoot reviews
  onApproveShoot?: () => void;
  onRejectShoot?: () => void;
  // Actions for edit reviews
  onApproveEdit?: () => void;
  onRejectEdit?: () => void;
...
const QuestionWithAudio = ({
  ...
  return (
    <div className="bg-white rounded-lg p-3 border border-gray-200">
      ...
      {selection && <p className="text-sm font-semibold text-gray-900 mb-1">{selection}</p>}
      {explanation && (
...
export default function ApprovalDetailPanel({
  ...
  // Render shoot review specific content
  if (detailType === 'shoot') {
    return (
      <div className="h-full flex flex-col bg-white">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
              <p className="text-xs font-medium text-gray-500 mb-1">Videographer</p>
              <p className="text-sm font-semibold text-gray-900">
                {analysis.videographer?.full_name ||
                  analysis.videographer?.email ||
                  'Unknown'}
              </p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
          ...
          {analysis.production_notes && (
            <div className="bg-amber-50 rounded-lg p-4 border border-amber-200">
              <p className="text-xs font-medium text-amber-700 mb-1">Production Notes</p>
              <p className="text-sm text-gray-800">{analysis.production_notes}</p>
            </div>
          )}
            ...
            {shootFiles.length > 0 ? (
              <div className="space-y-2">
                {shootFiles.map((file: ProductionFile) => (
                  <div
                    key={file.id}
                    className="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-lg hover:border-primary-300 transition"
                  >
                    ...
                    <a
                      ...
                      className="ml-3 px-3 py-1.5 text-xs font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 transition"
                    >
                      View
                    </a>
                  </div>
        ...
        <div className="flex-shrink-0 bg-white border-t border-gray-200 px-6 py-4">
          <div className="flex gap-3">
            <button
              onClick={onRejectShoot}
              disabled={isSubmitting}
              className="flex-1 inline-flex justify-center items-center px-4 py-3 border border-red-300 text-red-700 bg-red-50 rounded-lg hover:bg-red-100 font-medium transition disabled:opacity-50"
            >
              <XCircleIcon className="w-5 h-5 mr-2" />
              Request Reshoot
            </button>
            <button
              onClick={onApproveShoot}
              disabled={isSubmitting}
  ...
  // Render edit review specific content
  if (detailType === 'edit') {
    return (
      <div className="h-full flex flex-col bg-white">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
              <p className="text-xs font-medium text-gray-500 mb-1">Editor</p>
              <p className="text-sm font-semibold text-gray-900">
                {analysis.editor?.full_name || analysis.editor?.email || 'Unknown'}
              </p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
          ...
          {analysis.production_notes && (
            <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
              <p className="text-xs font-medium text-purple-700 mb-1">Editor Notes</p>
              <p className="text-sm text-gray-800">{analysis.production_notes}</p>
            </div>
          )}

          {/* Reference URL for viewing the edit */}
          {analysis.reference_url && (
            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
              <p className="text-xs font-medium text-blue-700 mb-2">Reference Video</p>
              <a
  ...
  // Render pending or approved script content (full analysis view)
  return (
    <div className="h-full flex flex-col bg-white">
      {/* Header */}
      <div className="flex-shrink-0 px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900">Analysis Details</h2>
          {onClose && (
            <button
              onClick={onClose}
        ...
        <div className="flex items-center space-x-2 mt-1">
          ...
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${getStatusStyles(analysis.status)}`}
          >
            {analysis.status}
          </span>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          >
            ...
            {analysis.sharing_number !== undefined && analysis.sharing_number !== null && (
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-3 border border-blue-200">
                ...
                <p className="text-2xl font-bold text-blue-900">{analysis.sharing_number}</p>
              </div>
            )}
          </CollapsibleSection>
        </div>
        ...
        <div className="space-y-3 pt-4 border-t border-gray-200">
          ...
          >
            <QuestionWithAudio
              question="1. Did the stop feel: Reflexive / Conscious / Weak pause?"
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 inline-flex justify-center items-center px-4 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" />
                  ) : (
      ...
      {detailType === 'approved' && onDisapprove && (
        <div className="flex-shrink-0 bg-white border-t border-gray-200 px-6 py-4">
          {!showDisapproveFeedback ? (
            <div className="space-y-3">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-sm text-blue-800">
                  This script is already approved. You can disapprove it to send it back for
                  revision.
                </p>
              </div>
              <button
                onClick={() => setShowDisapproveFeedback(true)}
                className="w-full inline-flex justify-center items-center px-4 py-3 border border-orange-300 text-orange-700 bg-orange-50 rounded-lg hover:bg-orange-100 font-medium transition"
              >
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 inline-flex justify-center items-center px-4 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 font-medium transition disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" />
                  ) : (
